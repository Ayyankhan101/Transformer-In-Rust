...
impl KVCache {
    ...
    pub fn append(&mut self, k_new: &Tensor, v_new: &Tensor) -> Result<(Tensor, Tensor)> {
        ...
        let k = self.k.slice_assign(&[0..1, 0..self.n_heads, self.pos..end, 0..self.head_dim], k_new)?;
        let v = self.v.slice_assign(&[0..1, 0..self.n_heads, self.pos..end, 0..self.head_dim], v_new)?;
