...
impl WeightLoader {
    pub fn load_from_pytorch(
        ...
        for i in 0..config.num_layers {
            ...
            if let Some(w) = load_tensor(
                &pth,
                &format!("transformer.h.{i}.attn.qkv_proj.weight"),
                dtype,
                device,
            ) {
                let w_t = w.transpose(0, 1)?;
                let out_w = load_tensor(
                    &pth,
            ...
            if let Some(fc_in) = load_tensor(
                ...
                let fc_in_b = load_tensor(
                    &pth,
                    &format!("transformer.h.{i}.mlp.fc_in.bias"),
                    dtype,
                    device,
                )
                .unwrap_or(Tensor::zeros(config.ffn_dim, dtype, device).unwrap());
                let fc_out = load_tensor(
                    &pth,
                ...
                let fc_out_b = load_tensor(
                    &pth,
                    &format!("transformer.h.{i}.mlp.fc_out.bias"),
                    dtype,
                    device,
                )
                .unwrap_or(Tensor::zeros(config.hidden_dim, dtype, device).unwrap());
                block.ffn = FeedForward::GELUNew(GELUNewFFN::from_tensors_with_bias(
                    fc_in_t, fc_in_b, fc_out_t, fc_out_b,
                ));
            }
        }
